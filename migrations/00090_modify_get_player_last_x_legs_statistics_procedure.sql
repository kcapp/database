-- +goose Up
DROP PROCEDURE IF EXISTS `get_players_last_x_legs_statistics`;
-- +goose StatementBegin
CREATE PROCEDURE `get_players_last_x_legs_statistics`(IN start INT, IN num_legs INT, IN weeks INT, IN ignore_match_id INT)
BEGIN
    ...
    FROM (
        ...
        WHERE m.is_finished = 1
          ...
          AND (IF(ignore_match_id IS NOT NULL, m.id <> ignore_match_id, TRUE))
        ORDER BY l.end_time DESC
    ) AS RankedLegs
    WHERE leg_rank > start AND leg_rank <= num_legs
    ...
    GROUP BY p.id
        HAVING legs_played >= (num_legs - start) AND last_played_leg >= NOW() - INTERVAL IFNULL(weeks, 100000) WEEK
...
DROP PROCEDURE IF EXISTS `get_players_last_x_legs_statistics`;
-- +goose StatementBegin
CREATE PROCEDURE `get_players_last_x_legs_statistics`(IN start INT, IN num_legs INT, IN weeks INT)
BEGIN
    -- Step 1: Create a temporary table to store last N legs per player
    CREATE TEMPORARY TABLE IF NOT EXISTS LastNLegsPerPlayer (
        player_id INT,
